...
Данную проблему, как и проблему, описанную в следующем негативном сценарии, предлагается решить следующим образом:

1. Добавить текущее состояние верифицированности прошивки доверенному Монитору безопасности.
2. Изменять данное состояние:

   * При обращении от Manager к Downloader на "неверифицировано".
   * При успешной верификации прошивки на "верифицировано".

3. Разрешить обращение от Manager к Updater только в том случае, когда состояние "верифицировано".

Таким образом, недоверенный Manager не сможет ни пропустить этап верификации, ни проигнорировать его результат.

Недостаток данного решения состоит в усложнении логики работы Монитора безопасности, у которого появляется состояние.
При расширении функциональности данной системы, например, если потребуется выполнять обновления различных устройств, возможно, параллельно, потребуется расширять состояние Монитора безопасности и проверять его более сложным образом.

Кроме того, текущая реализация системы и Монитора безопасности в частности не требуют обязательного обращения от Manager к Downloader.
Из-за этого недоверенный Manager может воспользоваться ранее достигнутым состоянием "верифицировано" для использования неверифицированной прошивки.
Данную проблему можно решить сбросом состояние в "неверифицировано" после обращения от Manager к Updater.
...
Данную проблему предлагается решить следующим образом: доверенный Updater должен запрашивать прошивку у доверенного Verifier.
При поступлении данного запроса Verifier должен тем или иным образом гарантировать безопасность прошивки.
Например, он может провести верификацию прошивки повторно.

Очевидно, что данное решение неэффективно, поскольку прошивка будет проверяться 2 раза раза.
Эту проблему можно решить, если устранить шаг с проверкой прошивки (обращение от Manager к Verifier).
Однако это потребует достаточно существенно переделать систему.
Кроме того, данное решение также, как и предыдущее, может иметь проблемы с масштабируемостью.

![Негативный сценарий3](./diagrams/docs/report/sd-negative-manager3/Hacked-manager3-fix.png?raw=true "Дополнительная проверка прошивки")
